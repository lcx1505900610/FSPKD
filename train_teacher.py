...
class Option:
    def __init__(self):
        ...
        parser.add_argument('--dataset_len', type=int, default=120000)#choices=[149428//2,55252]
...
def main(args):
    ...
    teacher = ModalityFusionNetwork(img_size, 3, feature_dim=teacher_out_dim, heads=3,num_class=num_class,
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=len(train_loader) * num_epochs, eta_min=0,
                                                           last_epoch=-1)
    loss_moco_sk=MOCO(K=int(batch_size*num_class*num_class//2),dim=teacher_out_dim).cuda()
    loss_moco_im=MOCO(K=int(batch_size*num_class*num_class//2),dim=teacher_out_dim).cuda()
