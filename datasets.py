...
class Teacherdatasets(Dataset):
    def __init__(self,file_path_list_path_p,file_path_list_path_s,batch_size,num_class,data_path):
        with open(file_path_list_path_p, 'r') as f:
            photo_path = f.readlines()
            f.close()
        with open(file_path_list_path_s, 'r') as f:
            sketch_path = f.readlines()
            f.close()
        ...
        for i in range(len(sketch_path)):
            sketch_, label = sketch_path[i].split(' ')
            photo_=photo_path[i].split('.')[0]+'.jpg'
            self.class_tree[int(label)].append([sketch_,photo_,int(label)])

        min_len=999999
        for i in range(num_class):
            if(len(self.class_tree[i])<min_len):min_len=len(self.class_tree[i])

        for _ in range(int(min_len/batch_size)):
            for j in range(num_class):
                self.result+=self.class_tree[j][_*batch_size:_*batch_size+batch_size]
    ...
    def __getitem__(self, index):
        sketch_,photo_,label= self.result[index]
        photo = cv2.imread(self.data_path +photo_)

        sketch= cv2.imread(self.data_path +sketch_)
        sketch= cv2.cvtColor(sketch, cv2.COLOR_BGR2RGB)
        sketch = remove_white_space_image(sketch, 10)
        sketch = resize_image_by_ratio(sketch, 224)
        sketch= make_img_square(sketch)

        photo, sketch= self.transform(photo), self.transform(sketch)
        return photo, sketch, label



if __name__ == '__main__':
    train_set, sk_valid_data, im_valid_data = load_data(args=Option().parse())
    print(train_set.get_num_class())
