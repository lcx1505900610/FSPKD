...
class Option:
    def __init__(self):
        ...
        parser.add_argument('--dataset_len', type=int, default=50000)#choices=[149428//2,55252]
...
def main():
    ...
    teacher = ModalityFusionNetwork(args.img_size, 3, feature_dim=args.teacher_out_dim, heads=3,encoder_backbone=args.teacher_encoder,
                                    num_class=num_class,
                                    checkpoint_path=args.teacher_pre_weight).cuda()
    teacher.load_state_dict(torch.load(f'../weights/teacher_{args.dataset}_good.pth'))
